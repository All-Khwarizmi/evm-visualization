...
export function GasProfiler({ gasUsed, gasLimit, opcodeGasCosts }: GasProfilerProps) {
  const gasPercentage = (gasUsed / gasLimit) * 100

  // Calculate total gas used by opcodes
  const totalOpcodeGas = opcodeGasCosts.reduce((sum, item) => sum + item.gas, 0)
      ...
      <CardContent>
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="text-xs text-right text-slate-500">{gasPercentage.toFixed(1)}% of gas limit consumed</div>
          </div>
